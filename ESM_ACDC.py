...
def the_model():
    ...
    N = 12000 # 3000 year run
    time = np.linspace(0,2999,N)
    ...
    F = np.zeros(N)
    #E = np.zeros(N)
    E = np.ones(N)*(2.5e12) #kgC 
    E[800:] = 0 # Experiment with 10 PgC per year for 200 years, then immediate cessation of emissions
    ...
    kappa = 4e-4                # turbulent diffusivity [m^2/s]
    ...
    while i < N-1:
        ...
        F[i] = F_G                                          # Total radiative forcing [W/m^2]
        ...
        dTA_dt = ((1/C_A)*(F[i] - (lam*T_A[i]))) - E_airsea                 # K/s
        ...
        #STEP
        i+=1
    return(time,T_A,c_A,T_O,pco2,DIC,F_AS*dt,F_LAND*dt,W_X)

### REPRODUCING TEXTBOOK FIGURE #############################
#T = np.linspace(273.15,303.15,100)
#S = np.linspace(30,40,100)
#pCO2 = np.zeros(shape=(100,100))
#i = 0
#DIC = 2090
#ALK = 2322
#while i < 100:
#	j = 0
#	while j < 100:
#		k0,k1,k2 = rate_constants(T[j],S[i])	
#		pCO2[i,j] = (44/29)*(k2)*(((2*DIC) - ALK)**2)/((k1*k0)*(ALK - DIC))
#		j+=1
#	i+=1

#print(np.nanmean(pCO2))
#bounds = np.linspace(100,500,41)
#plt.contourf(T-273.15,S,pCO2,levels=bounds)
#plt.colorbar()
#plt.show()

#f = breakhere
#k0,k1,k2 = rate_constants(T,35)
#plt.plot(T,k2)
#plt.show()


####### Revelle Factor Correct??? ###########
#K_0,K_1,K_2 = rate_constants(280.65,34.75)
#DICI = 2188*1e-6
#DICF = DICI*1.01
#ALK = 2322*1e-6

#pco2_F = (K_2)*(((2*DICF) - ALK)**2)/((K_1*K_0)*(ALK - DICF))
#pco2_I = ((K_2)/(K_0*K_1))*(((2*DICI) - ALK)**2)/(ALK - DICI)

#A_A = 5.1e14    # area of atmosphere [m^2]
#f_B = 0.15 #fraction of earth that's composed of biosphere
#P_A = 1000
#c_v,A,QP = GPP_Module(200,1,P_A*280e-06)
#print(f_B*A*(np.pi*1e7)*A_A/1e12/4)
#f = rbeareh

# test plot for rate constants

time,T_A,c_A,T_O,pco2,DIC,F_AS,GPP,W_X = the_model()
